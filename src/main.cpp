...
public:
    void run() {
        ...
        mainLoop();
        cleanup();
        LOG_INFO("Vulkan application terminated.");
    }
...
private:
    ...
    void initWindow() {
        ...
        LOG_INFO("GLFW window created: %dx%d", WIDTH, HEIGHT);
    }
    ...
    void cleanup() {
        LOG_INFO("Cleaning up Vulkan and GLFW resources...");

        cleanupSwapChain();
        LOG_DEBUG("Vulkan SwapChain cleaned up.");

        vkDestroyPipeline(device, graphicsPipeline, nullptr);
        LOG_DEBUG("Vulkan Graphics Pipeline destroyed.");

        vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
        LOG_DEBUG("Vulkan PipelineLayout destroyed.");
        ...
        LOG_DEBUG("Vulkan Semaphores & Fences destroyed.");

        vkDestroyCommandPool(device, commandPool, nullptr);
        LOG_DEBUG("Vulkan CommandPool destroyed.");
    ...
    void createInstance() {
        ...
        LOG_DEBUG("Vulkan instance created successfully with %zu extensions.", extensions.size());
    }
    ...
    void setupDebugMessenger() {
        ...
        LOG_DEBUG("Debug messenger setup complete.");
    }
    ...
    void pickPhysicalDevice() {
        ...
        LOG_INFO("Found %d physical device(s).", deviceCount);  
        ...
        for (const auto& device : devices) {  
            ...
            LOG_DEBUG("Device name: %s", deviceProperties.deviceName);  
            LOG_DEBUG("Device type: %d", deviceProperties.deviceType);  
            LOG_DEBUG("Vulkan API version: %d.%d.%d",  
                    VK_VERSION_MAJOR(deviceProperties.apiVersion),  
                    VK_VERSION_MINOR(deviceProperties.apiVersion),  
                    VK_VERSION_PATCH(deviceProperties.apiVersion));  
            LOG_DEBUG("Memory heaps: %d", memoryProperties.memoryHeapCount);  
            for (uint32_t i = 0; i < memoryProperties.memoryHeapCount; i++) {  
                LOG_DEBUG("  Heap %d size: %.2f MB, flags: 0x%x",  
                        i, memoryProperties.memoryHeaps[i].size / (1024.0f * 1024.0f),  
            ...
            if (indices.isComplete()) {  
                ...
                LOG_INFO("Selected physical device: %s", deviceProperties.deviceName);  
                break;  
            }  
        }  
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
        ...
        for (const auto& queueFamily : queueFamilies) {
            if (queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT) {
                ...
                LOG_DEBUG("Found graphics queue at index %d.", i);
            }
    ...
    std::vector<const char*> getRequiredExtensions() {
        ...
        LOG_DEBUG("Required extensions count: %zu", extensions.size());
        return extensions;
    }
    ...
    bool checkValidationLayerSupport() {
        ...
        for (const char* layerName : validationLayers) {
            ...
            if (!layerFound) {
                LOG_ERROR("Validation layer not found: %s", layerName);
                return false;
            }
        }
